 ...
 * Author: User
 *
 * Created on 23 de junio de 2024, 05:07 PM
 */
...
#include <cstdlib>
#include <ctime>
#define NUM_ANAQUELES 5
#define MAX_ALTURA 150
...
struct Caja {
    ...
    Caja(char id, int alt): id(id), altura(alt) {};
};
...
vector<Individual> init_population(int population_size, int chromosome_length) {
    ...
    for (int i = 0; i < population_size; ++i) {
        ...
        for (int j = 0; j < chromosome_length; ++j) {
            chromosome[j] = rand() % NUM_ANAQUELES;
        }
...
void evaluate_population(vector<Individual>& population, vector<Caja>& itemPool, int max_height) {
    for (Individual& ind : population) {
        calculate_fitness(ind, itemPool, max_height);
    }
}
...
void mutation_swap(Individual& ind) {
    int pos1 = rand() % ind.chromosome.size();
...
void genetic_algorithm(vector<Individual>& population, vector<Caja>& itemPool, int max_height, int generations, double mutation_rate, int tournament_size) {
    ...
    for (int g = 0; g < generations; ++g) {
        ...
        evaluate_population(offspring_population, itemPool, max_height); // evalua poblacion descendencia
